...
针对两个数组或者字符串的题目，$f(i,j)$可以表示成第一个的第i位和第二个的第j位的状态。

### 区间DP

区间dp的问题需要注意循环的方式，最外层是区间长度，其次是起始位置，最后是区间切分表达式的位置。
$$
区间dp\begin{cases} 状态表示:f(i,j) \begin{cases} 集合：所有将第i堆石子到第j堆石子合并成一堆石子的合并方式 \\ Min \end{cases} \\ f(i,j)=min(f(i,k)+f(k+1,j)-s[r]-s[l-1] (i<=k<j) \end{cases}
$$
