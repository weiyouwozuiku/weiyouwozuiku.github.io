...
- 锁主库，会导致备份期间不能执行除了查询之外的逻辑
- 锁从库，不能执行主库同步的binlog，导致主从延迟

官方自带的逻辑备份工具是mysqldump。当mysqldump使用参数–single-transaction的时候，导数据之前就会启动一个事务，来确保拿到一致性视图。而由于MVCC的支持，这个过程中数据是可以正常更新的。

针对不支持可重复读隔离级别的存储引擎，只能使用FTWRL。FTWRL在slaver上时，如果用户有超级权限readonly是无效的。
...
依赖各个引擎自己的实现。对于不支持行锁的引擎只能加表锁。

两阶段锁协议：**在InnoDB事务中，行锁是在需要的时候才加上，需要等到事务结束才会释放**。
